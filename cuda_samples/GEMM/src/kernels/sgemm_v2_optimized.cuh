...
template <
    const int BLOCK_ITEMS_M,   // Height in rows of a block-wide tile in matrix C
...
#pragma unroll
    for (int offset = 0; offset + A_TILE_ROW_STRIDE <= BLOCK_ITEMS_M;
        ...
        A_smem[0][A_tile_col_start + 1][A_tile_row_start + offset] =
            A_ldg_reg[ldg_reg_idx + 1];
        A_smem[0][A_tile_col_start + 2][A_tile_row_start + offset] =
            A_ldg_reg[ldg_reg_idx + 2];
        A_smem[0][A_tile_col_start + 3][A_tile_row_start + offset] =
...
#pragma unroll
            for (int offset = 0; offset + B_TILE_ROW_STRIDE <= BLOCK_ITEMS_K;
                ...
                FETCH_FLOAT4(B_ldg_reg[ldg_reg_idx]) = FETCH_FLOAT4(
                    B[OFFSET(block_tile_idx + B_tile_row_start + offset,  // row
                             B_tile_col_start,                            // col
                             N)]);
            }
        }
...
#pragma unroll
        for (int frag_k = 0; frag_k < BLOCK_ITEMS_K - 1; ++frag_k) {
            ...
            FETCH_FLOAT4(A_frag[(frag_k + 1) % 2][4]) = FETCH_FLOAT4(
                A_smem[read_stage_idx][frag_k + 1][A_smem_load_offset + 4 * 4]);
            // load B fragment
            FETCH_FLOAT4(B_frag[(frag_k + 1) % 2][0]) =
                FETCH_FLOAT4(B_smem[read_stage_idx][frag_k + 1][B_smem_load_offset]);
            FETCH_FLOAT4(B_frag[(frag_k + 1) % 2][4]) = FETCH_FLOAT4(
                B_smem[read_stage_idx][frag_k + 1][B_smem_load_offset + 8 * 4]);
...
#pragma unroll
            for (int offset = 0; offset + B_TILE_ROW_STRIDE <= BLOCK_ITEMS_K;
                 offset += B_TILE_ROW_STRIDE) {
                int ldg_reg_idx = offset / B_TILE_ROW_STRIDE * 4;
                FETCH_FLOAT4(B_smem[write_stage_idx][B_tile_row_start + offset]
                                   [B_tile_col_start]) =
